  ...
  return (
    <>
      <div className="accordion border border-bottom border-top-0 border-start-0 border-end-0 mb-3" id="accordionExample">
        ...
        <div className="card border-0">
          <div className="card-header px-0 py-4 bg-white border border-bottom-0 border-top border-start-0 border-end-0 rounded-0" 
               onClick={() => toggleAccordion('shirts')}>
            <div className="d-flex justify-content-between align-items-center pe-1">
              <h4 className="mb-0">服飾</h4>
              <i className={`fas fa-chevron-down ${openAccordion === 'shirts' || openAccordion === 'pants' ? 'rotate' : ''}`}></i>
            </div>
          </div>
          <div id="collapseTwo" className={`collapse ${openAccordion === 'shirts' || openAccordion === 'pants' ? 'show' : ''}`} aria-labelledby="headingTwo" data-bs-parent="#accordionExample">
            <div className="card-body py-0">
              <ul className="list-unstyled">
                <li>
                <Link to='/categoryshirts' className={`py-2 d-block text-muted ${activeAccordion === 'shirts' ? 'active' : ''}`}>所有服飾</Link>
                </li>
                <li>
                <Link to='/categorypants' className={`py-2 d-block text-muted ${activeAccordion === 'pants' ? 'active' : ''}`}>所有褲子</Link>
                </li>
              </ul>
            </div>
          </div>
        ...
        <div className="card border-0">
          <div className="card-header px-0 py-4 bg-white border border-bottom-0 border-top border-start-0 border-end-0 rounded-0" 
               onClick={() => toggleAccordion('goods')}>
            <div className="d-flex justify-content-between align-items-center pe-1">
              <h4 className="mb-0">配件</h4>
              <i className={`fas fa-chevron-down ${openAccordion === 'goods' ? 'rotate' : ''}`}></i>
            </div>
          </div>
          <div id="collapseThree" className={`collapse ${openAccordion === 'goods' ? 'show' : ''}`} aria-labelledby="headingThree" data-bs-parent="#accordionExample">
            <div className="card-body py-0">
              <ul className="list-unstyled">
                <li>
